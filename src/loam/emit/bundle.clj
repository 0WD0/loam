(ns loam.emit.bundle
  "Generic filesystem boundary for Loam compiled web bundles."
...
(defn- artifact-map [value]
  (cond
    ...
    :else (throw (ex-info "Bundle emitter requires successful compiler artifacts"
...
(def usage
  (str "Usage:\n"
       "  bb bundle --repo-root DIR --output-dir DIR [options] ENVELOPE.edn...\n"
       "  clojure -M -m loam.emit.bundle --repo-root DIR --output-dir DIR ENVELOPE.edn...\n\n"
       "Required:\n"
       ...
       "  --profile PROFILE      docs (default) or personal\n"))
...
(defn parse-cli-args
  "Parse generic bundle compiler flags and positional Envelope v1 files."
  [args]
      ...
      (let [argument (first args)]
        (if (str/starts-with? argument "--")
          (let [key (get cli-option-keys argument)
                value (second args)]
            (when-not key
              (throw (ex-info "Unknown Loam bundle compiler option"
                              {:argument argument :usage usage})))
            (when (or (nil? value) (str/starts-with? value "--"))
              (throw (ex-info "Missing Loam bundle compiler option value"
                              {:argument argument :usage usage})))
...
(defn- validate-cli-opts! [opts]
  (let [missing (vec (remove #(let [value (get opts %)]
    ...
    (when (seq missing)
      (throw (ex-info "Missing required Loam bundle compiler option"
...
(defn compile-cli!
  ...
  (let [opts (validate-cli-opts! opts)
        profile (case (:profile opts)
                  nil :loam/docs
                  "docs" :loam/docs
                  "personal" :loam/personal
                  :loam/docs :loam/docs
                  :loam/personal :loam/personal
                  (throw (ex-info "Unknown Loam bundle profile"
                                  {:profile (:profile opts) :usage usage})))
